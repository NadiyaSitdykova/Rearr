...
#PATH = "/home/nadya/Desktop/master/prepare_data/extended_vertebrate/grimm/"
PATH = "../grimm_with_repeat/"
GRIMM_FILES = list(map(lambda x: PATH + x, ["dog.txt", "opossum.txt", "cat.txt", "mouse.txt"]))
NEWICK_STRING_TREE = "((((human, chimp), (rat, mouse)), (dog, cat)), opossum);"
...
if __name__ == "__main__":
    ...
    print("Were identified",
          ...
          "assembly points")
    with open("logs/" + length + "_" + identity + "_assembly_log.txt", 'w') as destination:
        ...
        for v1, v2, weight, repeat_name, multicolor in result:
            for color in multicolor.colors:
                print(color.name, weight, v1.name, v2.name, "+" + repeat_name, file=destination)
