...
TARGETS = ["chimp"]
HISTORY_PATH = "/home/nadya/Desktop/master/prepare_data/extended_vertebrate/history_with_repeat/"
LOG_PATH = "/home/nadya/Desktop/master/rearr/evaluation/90+/vertebrate/only_repeats/7-chimp/logs/"
OUT_PATH = "/home/nadya/Desktop/master/rearr/evaluation/90+/vertebrate/only_repeats/7-chimp/stats/"
        ...
        with open(HISTORY_PATH + length + "_" + identity + "/" + genome + ".txt", 'r') as history_file:
            ...
            while line:
                ...
                if len(blocks) == 1: #end of file
                    ...
                    u = blocks[0]
                    v = blocks[2]
                    repeat_id = int(blocks[3])
                    if u not in history[name]:
                        history[name][u] = {}
                    if v not in history[name][u]:
                        history[name][u][v] = []
                    history[name][u][v].append(repeat_id)
    ...
    with open(LOG_PATH + length + "_" + identity + "_assembly_log.txt", 'r') as log_file:
        ...
        for line in log_file.readlines():
            ...
            if True:
                ...
                u = blocks[2]
                v = blocks[3]
                repeat_id = int(blocks[4])
                if name in history and u in history[name] and v in history[name][u] and repeat_id in history[name][u][v]:
