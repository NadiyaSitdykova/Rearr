...
count_of_steps = 24 # 24
count_of_iterations = 10
count_of_breakages_by_step = 50 #50
...
def step(n, g, names, complete_multiedges, cfg_file, path_to_mgra, subset):
    for _ in range(0, n):
        for i in subset:
...
def iteration(blocks_file, cfg_file, complete_multiedges, path_to_mgra, subset):
    g, names, y = [], [], []
    io_utils.read_genomes(blocks_file, g, names)
    y.append(step(0, g, names, complete_multiedges, cfg_file, path_to_mgra, subset))
    for _ in range(0, count_of_steps):
        y.append(step(count_of_breakages_by_step, g, names, complete_multiedges, cfg_file, path_to_mgra, subset))
    return y


def subset_processing(blocks_file, cfg_file, path_to_mgra, subset):
    seed(5)
    g, names = [], []
    io_utils.read_genomes(blocks_file, g, names)
    io_utils.write_genomes("blocks.txt", g, names)
    ...
    for i in range(0, count_of_iterations):
        print(i)
        ys.append(iteration(blocks_file, cfg_file, complete_multiedges, path_to_mgra, subset))
...
subsets = prepare_data.get_list_of_combinations(k, dataset)
subset_names = prepare_data.get_list_of_subsets(k, dataset)
ys = []
for subset in subsets:
    print("subset is: " + str(subset))
    ys.append(subset_processing("tmp.txt", "tmp.cfg", path_to_mgra, subset))

with open("mean.txt", 'w') as out:
    for i in range(0, len(ys)):
        s = ""
        print(subset_names[i], file=out)
        for j in range(0, len(ys[i])):
            s += str(round(ys[i][j], 4)) + " "
        print(s, file=out)
